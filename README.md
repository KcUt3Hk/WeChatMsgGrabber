解决本地破解微信消息导致的合规问题：模拟滑动聊天窗口查看消息，进行OCR识别，解析为结构化的消息。
...
v3.1版本更新
- 智能框选聊天窗口（如果失败，请重试）
- 尝试识别消息中的图片，但是识别率仍然很差，需要继续优化

因为还有其他事情要做，暂停更新一段时间。

v3.01版本更新
- 移除桌面应用中的几个参数（已作为默认值，不需要配置）
- UI细节优化
...
- 仅保留桌面应用
- 优化UI
<img width="500" height="1111" alt="image" src="https://github.com/user-attachments/assets/791572e3-c11c-4a0c-a874-6a05a3c6bd7f" />
...
结构化的消息生成后，请自行借助第三方AI（如 ChatGPT、Gemini 等）进行分析。
